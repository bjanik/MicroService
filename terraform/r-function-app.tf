resource "azurerm_function_app" "funapp" {
  ...
  app_settings = {
    ...
    "AzureWebJobsServiceBusConnection" = "@Microsoft.KeyVault(VaultName=${azurerm_key_vault.kv.name};SecretName=${azurerm_key_vault_secret.kvsecrets["SERVICE-BUS-CONNECTION-STRING"].name})"
  }
}